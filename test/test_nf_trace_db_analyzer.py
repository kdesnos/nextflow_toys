...
from nf_trace_db_analyzer import (
    analyze_process_execution_correlation,
    analyze_process_execution_time_consistency,
    anova_on_process_execution_times,
    get_execution_times_distribution_charasteristics,
    ...
    summarize_consistency_analysis,
...
class TestNextflowTraceDBAnalyzer(unittest.TestCase):
    ...
    def test_extract_execution_time_linear_reg_process(self):
        """
        ...
        result = extract_execution_time_linear_reg(self.db_manager, process_name, top_n=2, rmse_threshold=100000)
    ...
    def test_extract_execution_time_linear_reg_resolved_process(self):
        """
        ...
        result = extract_execution_time_linear_reg(
            self.db_manager,
    ...
    def test_extract_execution_time_quantile_reg_process(self):
        """
        Test the extract_execution_time_quantile_reg method with a regular process name.
        """
        # Test with regular process name
        process_name = "generate_binconfig"
        # Test with median (q=0.5)
        result = extract_execution_time_quantile_reg(self.db_manager, process_name, quantile=0.5, top_n=2, rmse_threshold=100000)
        ...
        self.assertIn("regression_type", result)

        # Check quantile-specific fields
        self.assertEqual(result["quantile"], 0.5)
        self.assertEqual(result["selection_method"], "linear_regression")
        self.assertEqual(result["regression_type"], "quantile")
        ...
        # Print some information for manual inspection
        print(f"\nQuantile regression (q=0.5) for process:")
        print(f"Expression: {result['expression']}")
        print(f"RMSE: {result['rmse']:.2f}")
        print(f"Parameters: {result['selected_parameters']}")

        # Test with upper quantile (q=0.9)
        result_upper = extract_execution_time_quantile_reg(
            self.db_manager,
            process_name,
            quantile=0.9,
            top_n=2,
        ...
        # Basic checks for the upper quantile result
        self.assertEqual(result_upper["quantile"], 0.9)
        self.assertIsInstance(result_upper["expression"], str)
        self.assertTrue(len(result_upper["expression"]) > 0)

        # Print upper quantile information
        print(f"\nQuantile regression (q=0.9) for process:")
        print(f"Expression: {result_upper['expression']}")
        print(f"RMSE: {result_upper['rmse']:.2f}")

    def test_extract_execution_time_quantile_reg_resolved_process(self):
        """
        Test the extract_execution_time_quantile_reg method with a resolved process name.
        """
        # Test with resolved process name
        resolved_process_name = "fcal1:corr_fcal:do_correlation"
        result = extract_execution_time_quantile_reg(
            self.db_manager,
            resolved_process_name,
            quantile=0.5,
            top_n=2,
        ...
        # Print some information for manual inspection
        print(f"\nQuantile regression (q=0.5) for resolved process:")
        print(f"Expression: {result['expression']}")
        print(f"RMSE: {result['rmse']:.2f}")
        print(f"Parameters: {result['selected_parameters']}")

        # Test with lower quantile (q=0.1)
        result_lower = extract_execution_time_quantile_reg(
            self.db_manager,
            resolved_process_name,
            quantile=0.1,
            top_n=2,
        ...
        # Basic checks for the lower quantile result
        self.assertEqual(result_lower["quantile"], 0.1)
        self.assertIsInstance(result_lower["expression"], str)
        self.assertTrue(len(result_lower["expression"]) > 0)

        # Print lower quantile information
        print(f"\nQuantile regression (q=0.1) for resolved process:")
        print(f"Expression: {result_lower['expression']}")
        print(f"RMSE: {result_lower['rmse']:.2f}")

        # Compare the different quantile models
        print("\nComparing different quantile models for the same process:")
        print(f"Median (q=0.5) expression: {result['expression']}")
        print(f"Lower (q=0.1) expression: {result_lower['expression']}")
        print("Parameters should be the same but coefficients may differ.")
